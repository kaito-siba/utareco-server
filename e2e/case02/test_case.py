...
def run_case02_test() -> bool:
        ...
        for j, song2 in enumerate(song_names):
            if i != j:  # 異なる楽曲の組み合わせ
                ...
                for mod_name, mod_hpcp in modified_hpcp[song2]:
                    ...
                    # 同一性判定
                    is_same = is_same_recording_advanced(
                        ref_hpcp, mod_hpcp, threshold=0.89
                    )
    ...
    print("-" * 80)
    print(
        f"{'リファレンス':^15} {'テスト対象':^25} {'判定':^10} {'期待値':^10} {'結果':^10}"
    )
    print("-" * 80)
    ...
    for ref_name, test_name, is_same, expected, passed in results:
        ...
        print(
            f"{ref_name:^15} {test_name:^25} {result_str:^10} {expected_str:^10} {status:^10}"
        )
